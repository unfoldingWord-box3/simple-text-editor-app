...
A list of **[Common Markers](#Markers)** are below, followed by both left-to-right and right-to-left **[Examples](#Examples)**, with a **[Book Code Reference](#Codes)** at the bottom of this page.

## <a name="Markers">Common Markers</a>
- `\c #` **c**hapter (required) - `\c`is followed by a space, then the chapter number in place of `#`, then another space, all on a line of its own.
- `\s#` **s**ection (optional) - `\s`is followed by a space, then the section number within the chapter in place of `#`, then the section title, all on a line of its own.
- `\v #` **v**erse (required) - `\v` is always placed at the very begining of a line, followed by a space, then the verse number in place of `#`, another space, and then the verse.
...
- `\mt#` **m**ajor **t**itle (name of the book) - `\mt#` goes in the heading section at the top of the file on a line of its own. The highest priority title is indicated by the number 1 in place of `#`, with any subtitles (optional) indicated by sequential numbers.
- `\id ABC` File identification (required) - `\id` is placed at the very begining of the first line, followed by a space, followed by the applicable 3 letter book code in place of `ABC`, followed by a space, optionally followed by additional information, which might be things like the name of the translation, the language name, versions number, date, or other items not specified elsewhere.
