...
- `\id ABC` File identification (required) - `\id` is placed at the very begining of the first line, followed by a space, followed by the applicable 3 letter book code in place of `ABC`, followed by a space, followed by additional file information if desired, which might be things like the name of the translation, the language name, versions number, date, or other items not specified elsewhere.
